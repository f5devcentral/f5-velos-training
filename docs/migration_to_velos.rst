...
F5 understands migrating configurations to new platforms can be a challenge and we've developed tooling that will help customers migrate existing BIG-IP configurations into VELOS tenants. Originally, F5 promoted the F5 BIG-IP Journeys application that assisted with migrating UCS based configurations from older platforms such as VIPRION to current TMOS versions running as tenants on the VELOS platforms.
